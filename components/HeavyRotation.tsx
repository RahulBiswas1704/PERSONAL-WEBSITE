  ...
  return (
    <div className="animate-fade-in-up h-full" style={{ animationDelay: "200ms" }}>
      ...
      <div className="flex flex-col gap-3">
        {tracks.map((track, i) => (
          <a key={i} href={track.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-4 p-3 rounded-xl border border-border/40 bg-background/30 hover:bg-neutral-50 dark:hover:bg-neutral-900/40 transition-colors group">
            ...
            {track.nowPlaying && (
              <div className="ml-auto opacity-100 transition-opacity pr-2">
                <div className="flex items-end gap-1 h-4">
                  <span className="w-1 rounded-full bg-accent h-full animate-[bounce_1s_ease-in-out_infinite_0ms]" />
                  <span className="w-1 rounded-full bg-accent h-2/3 animate-[bounce_1s_ease-in-out_infinite_150ms]" />
                  <span className="w-1 rounded-full bg-accent h-4/5 animate-[bounce_1s_ease-in-out_infinite_300ms]" />
                </div>
              </div>
            )}
          </a>
